...
\begin{prop}
	Assume the conditions in \cref{foliation-of-unstable-manifold} hold and that \(u = 1\). Let \(p \in \overline{M}_1\) be a fixed points under the flow. Then in some neighborhood of \(p\) in \(\R^n\), \(U\), there is \(C> 0 \) and \(\lambda > 0\) such that for any fixed point \(p'\in U\) and any \(q \in f^u(p)\) and \(q'\in f^u(p')\) we have
	\begin{equation}
...
\begin{proof}
	...
	\end{equation}
	Here the point \((x,0,0)\) picks out a point on \(\overline M_1\) and the one-dimensional foliation of the unstable manifold is parameterized by \(z\).
	Let \(p'\) be an arbitrary fixed point on \(\overline M_1\cap U\) and denote the local coordinates of \(p\) and \(p'\) by \((x,0,0)\) and \((x',0,0)\), respectively.
\end{proof}
