...
\begin{lem}
	Suppose \(F(\mu, z)\) are a family of \(C^1\) functions on \(I \times [0,\alpha]\), where \(I\) is a compact interval and \(F(\mu, 0) = 0\). Furthermore, assume
	\begin{equation}
		\frac{\partial F}{\partial z} (\mu , 0) > 0.
	\end{equation}
	Then for the family of of solutions to 
	\begin{equation}
		\begin{aligned}
			\dot z_\mu(t) &= F(\mu, z_\mu) 
		\end{aligned}
	\end{equation}
	there are constants \(C, \alpha > 0\) where 
	\begin{equation}
		|z_{\mu_1} (-t) - z_{\mu_2}(-t) | \leq C e^{-\alpha t}( |\mu_1- \mu_2| + |z_{\mu_1}(0) - z_{\mu_2}(0)|)
	\end{equation}
	for all \(\mu_1,\mu_2 \in I\) and \(t> 0\).
\end{lem}
