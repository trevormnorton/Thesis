...
\end{equation}
We will now assume \(\Phi_\mu\) has the form given in \cref{phi-taylor-series}. From the center manifold theorem, we have that the constant and first-order terms zero. Thus we start by first computing the second-order terms where \(\ell = 0, 1\). We get the following set of equations:
\begin{align}
	...
	\Phi^{(0)}_{101}  & = L_{\mu_0} \Phi^{(0)}_{011} \\
	\Phi^{(1)}_{010}  & = L_{\mu_0} \Phi^{(1)}_{001} \label{phi-1-001} \\
	\Phi^{(0)}_{110}  & = L_{\mu_0} \Phi^{(0)}_{101} \\
	2\Phi^{(0)}_{020}  & = L_{\mu_0} \Phi^{(0)}_{011} \\
	\Phi^{(0)}_{011}  & = L_{\mu_0} \Phi^{(0)}_{002} \\
	0 & = L_{\mu_0} \Phi^{(1)}_{100} \label{phi-1-100} \\
	0 & = L_{\mu_0} \Phi^{(0)}_{200} \label{phi-0-200}
\end{align}
\Cref{phi-1-100,phi-0-200} can be solved by noting that \(\zeta_0\) is the only zero eigenfunction for \(L_{\mu_0}\) and \(\zeta_0^*(\Phi_{100}^{(1)}) = \zeta_0^*(\Phi_{200}^{(0)}) = 0\) since \(\Phi_\mu\) takes values in \(\bbD_1\), thus \(\Phi_{100}^{(1)} =\Phi_{200}^{(0)} = 0\). Then \cref{phi-1-010} is reduced to 
\begin{equation}
...
One can also compute the cubic coefficient \(\Phi_{300}^{(0)}\) and get that
\begin{equation}
	\Phi_{300}^{(0)} = 0.
\end{equation}

We will not need to compute any of the other coefficients. As we will soon see, after a change of variables they end up being in the higher order terms to be neglected. Before proceeding, we will need a new parameterization for the center manifold. We let \(\epsilon > 0\) correspond with the amplitude of our travelling wave solution, and look to write \(\mu = c^{-2}\) in terms of \(\epsilon\). As seen in \cite{iooss2000travelling}, the heteroclinic orbits on the center manifold will only exist for \(c^2\) slightly less than \(1\). Based on some formal calculations, it appears \(c^2 = 1 - \epsilon^2/12\) will be the correct scaling. This will be borne out by the coming calculations. We have that \[\mu-1 = c^{-2} - 1 = \frac 1 {1-\epsilon^2/12} -1 = \frac {\epsilon^2}{12} +\mcO(\epsilon^4).\] Since we are looking for \(\epsilon\)-amplitude waves with wavelength of order \(\epsilon^{-1}\), we make the following change of variables:
\begin{equation}
...
\end{equation}
The differential equation for \(q\) comes form \cref{ode-for-q} and the fact that \(\zeta_1^*(\Phi_\mu) = 0\). Taking a derivative we also have
\begin{equation}
...
To match similar estimates made in \cite{friesecke1999solitary}, one would expect the remainder terms to also be in a Sobolev space like \(H^1\). This is in general not true. The travelling wave solution found above may not approach a different limit asymptotically than \(\epsilon \phi \), in which case the remainder does not approach zero asymptotically in space. A necessary condition to get \(R_{\epsilon,\pm}\in H^1\) would be for \(u_n\) to approach the same limits given as \(\pm \epsilon \phi(\epsilon (n-ct))\) as \(|n| \to \infty\).
...
\begin{restatable}{lem}{heteroclinicorbitsobolev}\label{heteroclinic-orbit-sobolev}
	Suppose that there is a \(C^{k}\) (\(k \geq 1\)) family of vector fields on \(\R^n\) that are  parameterized by \(\mu \in I\), where \(I\) is a compact interval. We take the parameterization to be \(C^1\). Assume that there is a family of heteroclinic orbits, \(\gamma_\mu\) that satisfy the following properties:
	\begin{enumerate}[label=(\roman*)]
		...
		\end{equation}
		\item There exists \(\sigma > 0\) such that 
		\begin{equation}
			\lim_{t\to\pm\infty} |\gamma_\mu(t) - \gamma_{\pm\infty}|e^{\sigma|t|} = 0, \quad \forall \mu \in I.
		\end{equation}
		\item The heteroclinic orbits vary continuously. That is, there is a \(C> 0\) such that for any \(\mu, \mu'\in I\)
		\begin{equation}
			|\gamma_\mu(t) -\gamma_{\mu'}(t) | \leq C |\mu - \mu'|, \quad \forall t \in \R.
		\end{equation}
	\end{enumerate}
	Then there exists \(C'>0\) such that for \(m = 1, 2, \ldots, k\) we have that 
	\begin{equation}\label{expoential-decay-derivatives}
		\lim_{t\to\pm \infty} |\gamma^{(m)}(t)| e^{\sigma|t|} = 0, \quad \forall \mu \in I
	\end{equation}
	and 
	\begin{equation}\label{continutity-of-derivatives}
		|\gamma^{(m)}_\mu(t) -\gamma^{(m)}_{\mu'}(t) | \leq C' |\mu - \mu'| \quad \forall t \in \R.
	\end{equation}
	Furthermore, the difference of heteroclinic orbits is also in \(H^k(\R; \R^n)\). There is \(C''>0\) such that for any \(\mu, \mu'\in I\)
	\begin{equation}
		\| \gamma_\mu - \gamma_{\mu'} \|_{H^k(\R;\R^n)} \leq C'' |\mu -\mu'|.
	\end{equation}
\end{restatable}
The proof of \cref{heteroclinic-orbit-sobolev} is given in \cref{lemma-appendix}.
