...
\begin{aligned}
	x_{\pm, \epsilon}(t+1) - x_{\pm, \epsilon}(t) &= \dot x_{\pm, \epsilon}(t+ 1/2) + \int_{t+1/2}^{t+1} \dddot{x}_{\pm, \epsilon}(s) (t+1 -s)^2\, ds \\
	 &= \pm \epsilon \phi(\epsilon (t+1/2)) + \epsilon^2 R_{\epsilon, \pm}(\epsilon (t+1/2))
\end{aligned}
\end{equation}
...
\begin{equation}
	u_n(t) = \pm \epsilon \phi_{\epsilon, \pm}(\epsilon(n-ct)) + \epsilon^2 R_{\epsilon, \pm}(\epsilon (n-ct))
\end{equation}
Additionally, if (H2) holds we improve the error estimate so that there are solutions of the form 
\begin{equation}
	u_n(t) = \pm \epsilon \phi(\epsilon(n-ct)) + \epsilon^3 R_{\epsilon, \pm}(\epsilon (n-ct)).
\end{equation}
